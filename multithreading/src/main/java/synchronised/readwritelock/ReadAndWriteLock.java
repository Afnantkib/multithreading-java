...
/**
 * If no lock is held:
 *     read lock can acquire ✅
 *     write lock can acquire ✅
 *
 * If read lock is held:
 *     another read lock can acquire ✅
 *     write lock cannot acquire ❌ waits
 *
 * If write lock is held:
 *     read lock cannot acquire ❌ waits
 *     another write lock cannot acquire ❌ waits
 * **/
public class ReadAndWriteLock {
    ...
    void read() throws InterruptedException {
        ...
        try {
            System.out.println(Thread.currentThread().getName() + " reading count -> " + counter.getValue());
        }
        finally {
            readlock.unlock();
        }
